...
export default function HubHeader() {
  ...
  return (
    <div className="w-full bg-white border-b border-slate-200 sticky top-16 z-30 shadow-xs">
      <div className="mx-auto max-w-[1200px] px-5 py-2.5 flex items-center justify-between">
        
        {/* 현재 허브 정보 라벨 */}
        <div className="hidden md:flex items-center gap-2 text-[11px] text-slate-400 font-bold uppercase tracking-wider">
            ...
            .map((sub) => {
              ...
              return (
                <Link
                  key={sub.id}
                  href={sub.href}
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold transition-all relative shrink-0 ${
                    isActive
