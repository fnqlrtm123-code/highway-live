...
export function getRoadList(): RoadProfile[] {
  ...
  const addRoad = (name: string, detail: string, type: 'highway' | 'urban' | 'national' | 'bridge') => {
    ...
    if (name.includes("경부고속도로")) {
      ...
      const verifyPool = [
        `본 페이지의 실시간 정체 수치는 국토교통부 국가교통정보센터(ITS) 및 각 지자체 교통정보망(TOPIS 등)의 신뢰성 높은 공공 데이터를 2~5분 간격으로 수집하여 실시간으로 반영하고 있습니다.`,
        `제공되는 통행 평속과 소통 원활/서행/정체 판단 정보는 도로 나들목(IC)과 주요 분기점에 배치된 교통량 검지기 센서의 실시간 실측값을 기반으로 하므로 높은 정확도를 자랑합니다.`,
        `지능형 교통시스템(ITS)의 신호 관제망 및 도로 관리청의 유고 정보를 유기적으로 연계하여, 주행 중 돌발 사고나 전방 정체 발생 시 휴식을 취할 수 있는 최적의 대피 경로와 휴게소 정보를 제공합니다.`
      ];
