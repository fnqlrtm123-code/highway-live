...
export function getRoadList(): RoadProfile[] {
  ...
  const addRoad = (name: string, detail: string, type: 'highway' | 'urban' | 'national' | 'bridge') => {
    ...
    if (name.includes("경부고속도로")) {
      ...
      const descPool = [
        `${name}(${start || '시작점'} ~ ${end || '도착지'})의 실시간 교통상황과 실시간 CCTV 영상을 확인할 수 있는 정보 페이지입니다. 도로 교통 흐름, 구간별 정체 상황 및 돌발 사고 정보와 함께 노선 내에 있는 방향별 휴게소 정보를 한눈에 조회할 수 있습니다.`,
        `${name}(노선 경로: ${detail})의 구간별 실시간 소통 속도와 교통 지체를 안내합니다. 운전 전에 주요 정체 구간과 실시간 CCTV 모니터링 정보를 확인하여 보다 빠르고 안전하게 이동 경로를 계획할 수 있습니다.`,
        `${name} 노선의 원활/서행/정체 구간 정보와 소통 평속, 실시간 교통 흐름 데이터를 제공합니다. 주행 시 혼잡이 발생하는 주요 교차로 및 접속로 현황과 우회 경로를 실시간으로 확인해 보시기 바랍니다.`
      ];
      description = descPool[descIndex];

      const verifyPool = [
        `${name}의 실시간 소통 및 정체 정보는 국토교통부 국가교통정보센터(ITS) 및 각 지방자치단체의 교통정보망 데이터를 실시간으로 수집·가공하여 2~5분 간격으로 신속하게 업데이트됩니다.`,
        `본 도로의 실시간 교통량 흐름은 주요 나들목(IC) 및 분기점(JC)에 설치된 교통량 검지기(VDS) 센서 정보와 주요 거점 평속 측정 데이터를 기반으로 분석하여 정확한 교통 정체 상태를 조회 및 제공합니다.`,
        `본 교통안내 페이지는 ${name} 노선의 실시간 소통 속도와 돌발 사고, 도로 공사 통제 정보 등 지능형 교통시스템(ITS)의 공공 데이터를 연계하여 실시간으로 갱신 및 표출합니다.`
      ];
      ...
      const detourPool = [
        ...
        `실시간 도로 전광판(VMS)의 소통 정보나 내비게이션 앱의 실시간 혼잡 예측 시간을 비교하여 본선 정체 꼬리가 길어질 시, 주요 교차로 진입 전에 국도나 일반 도로 우회 코스로 경로를 변경하는 것을 권장합니다.`,
      ...
      const faqPool = [
        ...
        [
          ...
          { q: "정체 정보를 활용하여 안전하게 이동하는 방법은 무엇인가요?", a: "출발 전 해당 노선의 실시간 교통상황판을 조회하여 지체 구간을 사전에 파악하고, 병목 구간 및 엇갈림 분기점 부근에서는 급작스러운 차선 변경을 피하고 충분한 차간 거리를 확보하며 양보 운전하는 것이 중요합니다." }
        ]
      ];
