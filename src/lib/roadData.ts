...
export function getRoadList(): RoadProfile[] {
  ...
  const addRoad = (name: string, detail: string, type: 'highway' | 'urban' | 'national' | 'bridge') => {
    ...
    if (name.includes("경부고속도로")) {
      ...
      const descPool = [
        `${name}${rangeText}의 교통상황과 CCTV 영상 확인 방법을 상세히 안내해 드립니다. 본 노선은 주요 권역을 연결하는 핵심 도로망으로, 주행 전 실시간 정체 상황을 미리 파악하면 이동 시간을 크게 단축할 수 있습니다.`,
      ...
      const verifyPool = [
        `${name}의 교통상황과 소통 정보는 국토교통부 국가교통정보센터(ITS) 및 지자체 교통정보센터(TOPIS 등)의 신뢰성 높은 데이터를 통해 2~5분 간격으로 실시간 수집 및 집계됩니다.`,
