...
export function getRoadList(): RoadProfile[] {
  ...
  const addRoad = (name: string, detail: string, type: 'highway' | 'urban' | 'national' | 'bridge') => {
    ...
    if (name.includes("경부고속도로")) {
      ...
      const verifyPool = [
        `${name}의 실시간 교통상황과 소통 정보는 국토교통부 국가교통정보센터(ITS) 및 지자체 교통정보센터(TOPIS 등)의 신뢰성 높은 데이터를 통해 2~5분 간격으로 실시간 수집 및 집계됩니다.`,
        `노선의 통행 평속과 구간별 원활, 서행, 정체 상태는 도로 나들목(IC) 및 분기점(JC) 주변에 설치된 교통량 검지기 센서의 실시간 측정값을 기반으로 분석되어 제공됩니다.`,
        `지능형 교통시스템(ITS) 및 도로 관리청의 실시간 사고, 공사, 통제 정보를 미러링하여 전방 정체 발생 시 유동적인 우회 도로 안내 및 인근 휴게소의 현황 정보를 제공합니다.`
      ];
