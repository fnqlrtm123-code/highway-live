...
export default function GasIndexPage() {
  ...
  const processedStations = useMemo(() => {
    ...
    if (fuelType === 'lpg') {
      list = list.filter(item => item.lpg !== null);
      ...
      <main className="mx-auto max-w-[1240px] px-4 py-12 flex-grow space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-2xs space-y-4">
            <h2 className="text-base font-black text-slate-900 border-b pb-3 border-slate-100 flex items-center justify-between">
              <span className="flex items-center gap-2">
                <span className="w-1.5 h-4 bg-blue-500 rounded-full inline-block"></span>
                오늘의 휘발유 최저가 TOP 3
              </span>
              <span className="text-[10px] text-slate-400 font-extrabold">전국 기준</span>
            </h2>
            <div className="space-y-3">
              {cheapestGasoline.map((area, idx) => (
                <div key={idx} className="flex justify-between items-center text-xs">
                  <div className="space-y-0.5">
                    <span className="font-extrabold text-slate-800">{idx + 1}위. {area.name} ({area.directionName})</span>
        ...
        <section className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-2xs space-y-6">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b pb-4 border-slate-100">
            <div>
              ...
              <p className="text-slate-400 text-[11px] font-bold mt-1">유종을 선택하고 검색어를 입력하여 맞춤형 가격표를 확인하세요.</p>
            </div>

            {/* 유종 토글 스위치 */}
            <div className="flex bg-slate-100 p-1 rounded-xl text-xs font-black text-slate-600 shrink-0 self-start lg:self-center">
              <button 
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5">
                          <span className="font-black text-slate-800">{station.name}</span>
                          <span className="bg-slate-200/60 text-slate-600 text-[9px] font-black px-1.5 py-0.5 rounded-sm">
                            {station.brand}
                          </span>
                        </div>
                      ...
                      <div className="text-right flex items-center gap-3">
                        <span className={`font-mono font-black text-sm ${
                          fuelType === 'gasoline' ? 'text-blue-600' : fuelType === 'diesel' ? 'text-orange-600' : 'text-emerald-600'
                        }`}>
                          {price ? `${price.toLocaleString()}원` : '정보 없음'}
                        </span>
                        <a 
                          href={`/rest-areas/${station.slug}/gas`}
                          className="bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-black px-2.5 py-1.5 rounded-md transition-colors"
                        >
                          가격 추이 &rarr;
                        </a>
                      </div>
        ...
        <section className="bg-slate-900 text-white rounded-3xl p-6 md:p-8 space-y-6">
          ...
          <div className="space-y-5 text-xs md:text-sm text-slate-400 leading-relaxed font-medium">
            <div className="space-y-1">
              <h4 className="font-bold text-white text-[14.5px]">Q. 알뜰주유소(ex-OIL)와 브랜드 주유소의 요금 차이가 왜 발생하나요?</h4>
              <p>
            ...
            <div className="space-y-1 border-t border-slate-800 pt-4">
              <h4 className="font-bold text-white text-[14.5px]">Q. 전기차(EV) 급속/초급속 충전소의 요금 및 결제는 어떻게 되나요?</h4>
              <p>
