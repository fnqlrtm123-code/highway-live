          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-4 border-slate-200">
            ...
            <div className="relative w-full md:w-80">
              <input
                ...
                className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-4 py-2 text-xs font-bold text-slate-800 placeholder-slate-400 focus:outline-hidden focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
              />
        ...
        <section className="space-y-6">
          <div className="flex justify-between items-baseline">
            <h3 className="text-base font-black text-slate-800">
              조회 결과 <span className="font-mono text-blue-600">{filteredServiceAreas.length}</span>곳
            </h3>
          ...
          {filteredServiceAreas.length === 0 ? (
            <div className="bg-white border border-slate-150 rounded-2xl py-16 text-center text-slate-400 font-bold text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredServiceAreas.map((s) => (
                <div 
                  key={s.slug} 
                  className="p-5 border border-slate-200/60 bg-white hover:border-blue-500 hover:shadow-xs transition-all rounded-2xl flex flex-col justify-between space-y-4"
                >
                  <div className="space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-[17px] font-black text-slate-900">{s.name}</h4>
                    ...
                    <div className="bg-slate-50 p-3.5 border border-slate-100 rounded-xl space-y-1">
                      <div className="flex justify-between items-baseline">
                        <span className="text-[10.5px] font-extrabold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded">시그니처 메뉴</span>
                        <span className="text-[13.5px] font-mono font-black text-slate-900">{s.signatureMenu.price.toLocaleString()}원</span>
                      </div>
                      <p className="text-[14px] font-black text-slate-800 mt-1">{s.signatureMenu.name}</p>
                      <p className="text-xs text-slate-400 font-medium line-clamp-1">{s.signatureMenu.description}</p>
                    </div>
                  </div>

                  {/* 하단 편의시설 칩 및 상세정보 링크 */}
                  <div className="flex justify-between items-center pt-3 border-t border-slate-100/70">
                    <div className="flex flex-wrap gap-1 max-w-[220px]">
                      {s.facilities.slice(0, 3).map((f) => (
                        <span key={f} className="text-[9.5px] font-extrabold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md">
                          {f}
                        </span>
                      ))}
                      {s.facilities.length > 3 && (
                        <span className="text-[9.5px] font-black text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded-md">
                          +{s.facilities.length - 3}
                        </span>
                      )}
                    </div>
                    <a 
                      ...
                      className="bg-slate-900 text-white hover:bg-slate-800 font-black text-xs px-3.5 py-2 rounded-xl transition-colors shrink-0"
                    >
                      종합 정보 &rarr;
                    </a>
                  </div>
        ...
        <section className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-2xs space-y-6">
          ...
          <div className="space-y-4 text-xs md:text-sm text-slate-700 leading-relaxed font-medium">
            <div className="space-y-1.5">
              <h4 className="font-black text-slate-900 text-sm">Q. 한국도로공사의 EX-FOOD 인증 메뉴는 무엇인가요?</h4>
            ...
            <div className="space-y-1.5 border-t border-slate-100 pt-4">
              <h4 className="font-black text-slate-900 text-sm">Q. 휴게소 수유실, 화장실, 수면실 및 샤워시설 등 편의시설 유무는 어떻게 파악하나요?</h4>
              <p className="text-slate-500">
            ...
            <div className="space-y-1.5 border-t border-slate-100 pt-4">
              <h4 className="font-black text-slate-900 text-sm">Q. 상행선 휴게소와 하행선 휴게소 음식이 다른가요?</h4>
              <p className="text-slate-500">
