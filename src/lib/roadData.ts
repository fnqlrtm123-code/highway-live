...
export function getRoadList(): RoadProfile[] {
  ...
  const addRoad = (name: string, detail: string, type: 'highway' | 'urban' | 'national' | 'bridge') => {
    ...
    if (name.includes("경부고속도로")) {
      ...
      verificationMethod = "서해안선의 정체 정보는 아산만 관제 포인트 및 서해대교 교량 내부 루프 센서 검지 시스템의 실시간 진행 속도 데이터를 활용합니다.";
      ...
      verificationMethod = "올림픽대로의 소통 지표는 서울특별시 교통정보센터(TOPIS) 및 도로 관리 공단의 실시간 주행 센서 데이터를 바탕으로 평속을 집계해 산출합니다.";
      ...
      verificationMethod = "강변북로의 통행 평속은 서울시 TOPIS 실시간 정보망 및 각 대교 북단 진출입 톨링 데이터를 기반으로 신속하게 계산되어 반영됩니다.";
      ...
      const descPool = [
        `${name}은(는) 기점인 ${start || '시작점'}과 종점인 ${end || '도착지'} 구간을 양단으로 잇는 교통 벨트로서, 인프라 수송 및 지역 주민 통행 권역을 지원하는 중요 ${typeText} 노선입니다. 본 노선의 소통 정보를 주행 전 확인하십시오.`,
        `${name}은(는) 대한민국 도로 교통 네트워크 상에서 ${typeText} 역할을 부여받았으며, 출퇴근 비즈니스 차량 및 여가 유동량 비중이 일관되게 작용하는 핵심 소통로입니다.`,
        `${name}(노선 경로: ${detail})은(는) 구간 연장 효율성을 유지하며 타 주요 간선 도로망과 거미줄처럼 엮여 유기적인 도심 물류를 제공하는 소통 축입니다.`
      ];
      description = descPool[descIndex];

      const verifyPool = [
        `${name}의 실시간 소통 및 정체 수치는 국토교통부 국가교통정보센터(ITS) 및 지방자치단체의 모바일 지능형 소통 데이터 피드를 수집하여 2~5분 간격의 실시간 평속을 기준으로 분석 및 표출합니다.`,
        `본 도로의 소통 흐름은 나들목(IC)에 구비된 영상 차량 센서 측정 정보 및 톨링 게이트 통과 진행 속도 검측값을 복합 계산하여 정확하게 조회 반영하고 있습니다.`,
        `본 교통안내 시스템은 ${name} 노면을 횡단하는 통행량의 누적 흐름과 돌발 유고 소식을 도로 관리청 지능형 시스템(ITS) 전송 네트워크를 거쳐 즉각 받아 갱신 처리합니다.`
      ];
      ...
      const detourPool = [
        `만약 본 노선이 사고나 병목 지정체로 인해 평균 속도가 30km/h 이하로 저하되어 소통 흐름이 원활하지 못할 경우, 직전 나들목에서 이탈하여 노선에 근접한 대체 국도나 지방 일반 노선으로 선회하여 정체 주차장 대기를 피하는 것이 이롭습니다.`,
        `도로변의 가변 전광 표지판(VMS) 안내문이나 도로 감지 모바일 내비게이션의 소요 연산 시간을 비교 체크하여 정체 꼬리가 3km 이상 누적된 본선을 피해 신호 대기 우회 코스로 진로를 변경해 주는 것이 현명합니다.`,
        `교차로 지체가 지나치게 장기화되어 20분 이상의 통과 시간 손실이 일어나는 경우에는 유도 표지판의 지시를 참고하여 즉각 인근 일반 지방도로나 외곽 순환 우회로로 진출할 기준을 세워야 합니다.`
      ];
      ...
      const faqPool = [
        ...
        [
          ...
          { q: "정체 분석 정보를 가장 영리하게 활용하는 운전 습관은?", a: "본선 진입 전 미리 대시보드 속도를 체크하고, 진출입 혼잡 인터체인지에서 무리한 깜빡이 끼어들기를 예방하는 양보 주행을 체득하는 것입니다." }
        ]
      ];
